...
# Build
#  setwd(R'(C:\Users\James.Thorson\Desktop\Git\tinyVAST)'); devtools::build_rmd("vignettes/mgcv.Rmd")
# PDF
#  library(rmarkdown); render( "vignettes/mgcv.Rmd", pdf_document())
```
...
```{r, eval=TRUE, echo=TRUE, message=FALSE, fig.width=6, fig.height=6}
predict(out, newdata=data.frame(x=0, y=1, w=0.4, group=2) )
```
...
```{r, eval=TRUE, echo=TRUE, message=FALSE, fig.width=6, fig.height=6}
visreg(out, xvar="group")
visreg(out, xvar="x")
visreg(out, xvar="w", by="group")
```
