...
```{r, eval=TRUE, echo=TRUE, message=FALSE, fig.width=6, fig.height=6}
R1 = sum( residuals.tinyVAST(out, type="deviance")^2 )

# tinyVAST null model with just a single intercept
null = fit( data = Data, formula = n ~ 1 )
R0 = sum( residuals.tinyVAST(null, type="deviance")^2 )
...
R1_reduced = sum( residuals.tinyVAST(out_reduced, type="deviance")^2 )
1 - R1_reduced/R0
