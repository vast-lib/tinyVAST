...
# Bivariate genearlized linear mixed model structure

tinyVAST is a generalized linear mixed model, which includes four additive components:
...
$$
\begin{aligned}
p_{\PL 1,i} &= \B X_{\PL 1,i} \B \alpha_{\PL 1} + \B Z_{\PL 1,i} \B \gamma_{\PL 1} + \B A_{i} \B \Omega_{\PL 1,c[i]} + \B D_{\PL 1,c[i],t[i]} + \B A_i \B E_{\PL 1,c[i],t[i]}  \\
p_{\PL 2,i} &= \B X_{\PL 2,i} \B \alpha_{\PL 2} + \B Z_{\PL 2,i} \B \gamma_{\PL 2} + \B A_{i} \B \Omega_{\PL 2,c[i]} + \B D_{\PL 2,c[i],t[i]} + \B A_i \B E_{\PL 2,c[i],t[i]}
\end{aligned}
$$
...
-   $\B A_{i} \B \Omega_{\PL 1,c[i]}$ is the projection of the space-veriable interaction $\B \Omega$ to sample $i$;
...
-   $p_{\PL 2,i}$ is the second linear predictor, and terms are defined similarly but using subscript-2;

The linear predictors are then passed through an inverse-link function to specify the distribution for errors:
...
-   $\theta_{e[i]}$ is the dispersion parameters for distribution $e[i]$;

In the simple case, the distribution only requires a single linear predictor such that $p_{\PL 2,i}$ drops out of the model.  In this case, the model collapses to a generalized linear mixed model.  For example we might have a log-linked Poisson distribution, $y_i \sim \mathrm{Poisson}(e^{p_{\PL 1,i}})$.
...
-   $\mu_{\PL 1,i}$ and $\mu_{\PL 2,i}$ are the two linear predictors, such that $\mathbb{E}[y_i] = \mu_{\PL 1,i} \mu_{\PL 2,i}$;
...
$$
where $\B A$ is the adjacency matrix specified by the analyst.
...
This then estimates a single slope parameter.  In a more complicated case, $W_1$ might cause $W_2$, which in turn causes $W_3$.  This is then represented as:
...
These path coefficients then define path matrix $\B P$ and exogenous covariance matrix $\B G^2$, which is  used in a simultaneous equation model

$$
\mathbf{ w = P w + \epsilon} \\
\B \epsilon \sim \mathrm{MVN}( \B 0, \B G^2 )
$$
where the variance $\mathrm{Var}(\B w) = (\mathbf{I - P})^{-1} \B G^2 (\mathbf{I - P}^T)^{-1}$.  This then results in a sparse precision matrix:

$$
\B Q = (\mathbf{I - P}^T) \B G^{-2} (\mathbf{I - P})
$$

# Dynamic structural equation models

Similarly, tinyVAST involves specifying dynamic structural equation models (DSEM).  To specify a DSEM, the user uses *arrow-and-lag notation*.  For example, to specify a first order autoregressive process:
...
tinyVAST then uses the SEM and DSEM notation to construct the joint precision for the space-variable interaction $\B \Omega$ with dimension $S \times C$, and the space-time-variable interaction $\B E$ with dimension $S \times C \times T$.  To do so, it constructs the separable precision for each process:
