...
We first explore the ability to specify a first-order autoregressive spatio-temporal process, i.e., a spatial Gompertz model [@thorson_importance_2014].  
...
Next, we compare this against the current version of VAST [@thorson_comparing_2017]
...
Or with sdmTMB [@anderson_sdmtmb_nodate]
...
We can also fit this univariate spatio-temporal process using a Poisson-linked gamma delta model [@thorson_three_2018]
...
And we can again use the `DHARMa` package [@hartig_dharma_2017] to visualize conditional simulation quantile (a.k.a. Dunn-Smythe) residuals [@dunn_randomized_1996]:
...
We next highlight how to specify a bivariate spatio-temporal model with a cross-laggged (vector autoregressive) interaction [@thorson_estimating_2017,@thorson_spatio-temporal_2019]. 
## Simulate bivariate model
