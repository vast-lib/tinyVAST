# This file contains functions derived from the 'sem' package.
          ...
          quiet = FALSE)
{
    add.variances <- function() {
        if (!quiet)
            message("NOTE: it is generally simpler to use specifyEquations() or cfa()\n",
        ...
        model.2 <- cbind(c(model[, 1], paths), c(model[, 2],
            par.names), c(model[, 3], rep(NA, length(paths))))
    ...
    model <- if (!missing(text))
        scan(text = text, what = list(path = "", par = "", start = 1,
            dump = ""), sep = ",", strip.white = TRUE, comment.char = "#",
            fill = TRUE, quiet = quiet)
    else scan(file = file, what = list(path = "", par = "", start = 1,
        dump = ""), sep = ",", strip.white = TRUE, comment.char = "#",
        fill = TRUE, quiet = quiet)
    ...
    if (!(missing(covs))) {
        for (cov in covs) {
            ...
            for (i in 1:nvar) {
                for (j in i:nvar) {
                  row <- c(paste(vars[i], "<->", vars[j]), if (i ==
                    j) paste("V[", vars[i], "]", sep = "") else paste("C[",
                    vars[i], ",", vars[j], "]", sep = ""), NA)
                  if (row[2] %in% model[, 2])
                    next
    ...
    result[which.pars, 2] <- paste(result[which.pars, 2], suffix,
        sep = "")
    result
}
...
specifyEquations <-function (file = "", text, ...)
{
    ...
    equations <- if (!missing(text))
        scan(text = text, what = "", sep = ";", strip.white = TRUE,
            comment.char = "#")
    else scan(file = file, what = "", sep = ";", strip.white = TRUE,
        comment.char = "#")
...
function ( model, warn = TRUE)
{
    ...
    vars <- matrix(unlist(strsplit(paths, "<->|<-|->")), ncol = 2,
        byrow = TRUE)
    ...
    if (warn && any(dupl.paths)) {
        warning("the following duplicated paths were removed: ",
            paste(model[dupl.paths, 1], collapse = ", "))
    }
