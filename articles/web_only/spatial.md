...
We can then calculate the area-weighted total abundance by supplying a
`newdata` object that has rows for each area in the domain. Here however
we show a simpler sample-weighted index:
...
Sys.time() - start_time
#> Time difference of 0.03311276 secs
...
Runtime for this vignette: 4.88 secs
