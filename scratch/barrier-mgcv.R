...
tvgrid$tv = NA
tvgrid$tv = predict(tv, newdata = tvgrid)
...
tvgrid$tvb = NA
tvgrid$tvb = predict(tvb, newdata = tvgrid)

cols <- RColorBrewer::brewer.pal(3, "BrBG")
p1 <- ggplot(tvgrid, aes(x, y, fill = tv)) +
  geom_raster() +
  geom_point(data = dat, aes(x, y, size = z), inherit.aes = FALSE, pch = 21) +
  scale_fill_gradient2(high = cols[1], low = cols[3], mid = cols[2]) +
  theme(legend.position = "none") +
...
p2 <- ggplot(tvgrid, aes(x, y, fill = tvb)) +
  geom_raster() +
  geom_point(data = dat, aes(x, y, size = z), inherit.aes = FALSE, pch = 21) +
  scale_fill_gradient2(high = cols[1], low = cols[3], mid = cols[2]) +
  theme(legend.position = "none") +
...
cowplot::plot_grid(g1, g2, p1, p2, ncol = 2)
